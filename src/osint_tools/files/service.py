...
def ingest_file(store: Store, content_store: ContentStore, case_id: int, stream: BinaryIO, content_length: int, filename: str, max_bytes: int) -> dict:
    ...
    try:
        return store.add_file_ingestion(case_id, filename, analysis)
    except Exception:
        if created:
