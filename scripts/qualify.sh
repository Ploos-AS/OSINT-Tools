...
run_gate "secret-redaction regression" python -m pytest -q tests/test_providers.py tests/test_provider_api.py
...
assert i["version"] == "0.3.0" and i["milestone"] == "M3.1"
PY
...
r = IPInfoProvider().execute("lookup", {"normalized": "1.1.1.1"}, timeout=8)
assert isinstance(r.data, dict)
PY
    then record "live provider call" PASS ""; else record "live provider call" FAIL "credential present but bounded call failed"; fi
else
    record "live provider call" SKIPPED "IPINFO_TOKEN is not set"
fi
