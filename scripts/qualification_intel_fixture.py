...
# Foreign authorization-like fields are hostile input, never local policy.
AUTH_FIELDS={"owner_user_id":1,"owner":1,"acl":[{"principal_type":"user","principal_id":1,"access":"editor"}],"teams":[{"name":"injected"}],"users":[{"username":"injected"}],"roles":["admin"],"sessions":["injected"],"auth_config":{"enabled":False}}
STIX.update(AUTH_FIELDS); STIX['objects'][0].update(AUTH_FIELDS)
MISP.update(AUTH_FIELDS); MISP['Event'].update(AUTH_FIELDS)
